---
Title: Attachment Field Configuration - 8.9.0+
weight: 20
---
...
To understand how the media uploader and storage system works, please refer to the link:../8.9.0-file-storage-configuration/[File Storage Configuration Reference].

It is also important to know the link:../8.9.0-media-object-lifecycle[lifecycle of media objects] to understand how files are managed in the system.
...
| `storage_type`
| The storage backend to use for this field. Must match a configured storage (e.g., `private-documents`, `public-images`). See the link:../8.9.0-file-storage-configuration/[File Storage Configuration Reference].
| Yes
...
The `storage_type` must correspond to a storage defined in your media configuration (see link:../8.9.0-file-storage-configuration/[File Storage Configuration Reference]). Example storage types include:
